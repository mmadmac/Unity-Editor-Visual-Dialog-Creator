...
public class DialogueGraphEditor_Step6_SaveLoad : EditorWindow
{
    ...
    public static void Open() =>
        GetWindow<DialogueGraphEditor_Step6_SaveLoad>("Dialogue Editor");
    ...
    private void DrawNode(DialogueNodeData node)
    {
        ...
        {
            fontSize = Mathf.RoundToInt(12 * zoomLevel)
        };
        ...
        switch (node.type)
        {
            case NodeType.Dialogue:
                ...
                node.text = EditorGUILayout.TextArea(
                    node.text,
                    GUILayout.MinHeight(40 * zoomLevel)
                );
            ...
            case NodeType.Condition:
                node.variableName =
                    EditorGUILayout.TextField("Item/Var:", node.variableName);
                node.requiredValue =
                    EditorGUILayout.IntField("Cantidad:", node.requiredValue);
            ...
            case NodeType.InventoryEvent:
                node.inventoryAction =
                    (InventoryAction)EditorGUILayout.EnumPopup(node.inventoryAction);
                node.itemName =
                    EditorGUILayout.TextField("Item:", node.itemName);
                node.requiredValue =
                    EditorGUILayout.IntField("Cant:", node.requiredValue);
            ...
            case NodeType.SpriteEvent:
                GUILayout.Label("Sprite Event", EditorStyles.miniBoldLabel);

                node.imageIndex =
                    EditorGUILayout.IntField("Image Index:", node.imageIndex);
    ...
    private void DrawNodeOptions(DialogueNodeData node, bool isDialogue)
    {
        for (int i = 0; i < node.options.Count; i++)
        {
            ...
            if (isDialogue)
                node.options[i].text =
                    EditorGUILayout.TextField(node.options[i].text);
            else
                node.options[i].chance =
                    EditorGUILayout.IntField(
                        node.options[i].chance,
                        GUILayout.Width(40 * zoomLevel)
                    );

            if (GUILayout.Button("→", GUILayout.Width(25 * zoomLevel)))
            {
                fromNodeId = node.id;
                fromOptIdx = i;
            }

            if (GUILayout.Button("x", GUILayout.Width(20 * zoomLevel)))
            {
        ...
        if (GUILayout.Button("+ Opción"))
            node.options.Add(
                new DialogueOptionData { text = "...", chance = 50 }
            );
    }
    ...
    private void DrawConnections()
    {
        ...
        foreach (var n in nodes)
        {
            for (int i = 0; i < n.options.Count; i++)
            {
                ...
                var target =
                    nodes.Find(t => t.id == n.options[i].targetNodeId);
    ...
    private void DrawPreviewLine()
    {
        ...
        Handles.DrawBezier(
            ...
            (Vector3)Event.current.mousePosition +
                Vector3.left * 50 * zoomLevel,
            Color.yellow,
            null,
            2f * zoomLevel
        );
    ...
    private Vector3 GetOutputPos(DialogueNodeData n, int optIdx)
    {
        ...
        if (n.type == NodeType.InventoryEvent || n.type == NodeType.SpriteEvent)
    ...
    private void HandleEvents()
    {
        ...
        if (e.type == EventType.ScrollWheel)
        {
            ...
            zoomLevel =
                Mathf.Clamp(
                    zoomLevel - e.delta.y * 0.02f,
                    minZoom,
                    maxZoom
                );
            panOffset +=
                (e.mousePosition / oldZ -
                 e.mousePosition / zoomLevel);
            e.Use();
        }
        ...
        {
            DialogueNodeData hit =
                nodes.LastOrDefault(
                    n => ScaleRect(new Rect(n.position, n.size))
            ...
            if (hit != null)
            {
                if (fromNodeId != -1 && fromNodeId != hit.id)
                {
                    ...
                    e.mousePosition.y <
                    ScaleRect(new Rect(hit.position, hit.size)).y +
                    25 * zoomLevel
                )
                {
                    draggingNode = hit;
    ...
    void DrawGrid(float spacing, float alpha)
    {
        ...
        for (float x = ox; x < position.width; x += s)
            Handles.DrawLine(
                new Vector3(x, 0),
                new Vector3(x, position.height)
            );

        for (float y = oy; y < position.height; y += s)
            Handles.DrawLine(
                new Vector3(0, y),
                new Vector3(position.width, y)
            );
    ...
    void AddNode(NodeType type)
    {
        ...
        if (type == NodeType.Start ||
            type == NodeType.InventoryEvent ||
            type == NodeType.SpriteEvent)
    ...
    {
        string path =
            EditorUtility.OpenFilePanel("Load", "Assets", "asset");
        if (string.IsNullOrEmpty(path)) return;

        loadedAsset =
            AssetDatabase.LoadAssetAtPath<DialogueGraphAsset>(
    ...
    DialogueNodeData CloneNode(DialogueNodeData n)
    {
